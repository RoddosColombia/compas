...
- [ ] Deploy a staging por merge a `main` funcionando; deploy a producción BLOQUEADO. **Control (repo Free, decisión CEO 20-jul):** Render `compas-api` con `autoDeploy:false` + tags `v*` creables solo por el CEO (único admin del repo) + deploy manual. El *required reviewer nativo* de GitHub necesita plan **Team** (repo privado Free → HTTP 422); se difiere hasta que el CEO decida subir de plan (entonces reviewer = CEO, patrón CR-003)
