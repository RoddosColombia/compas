...
| **P3** | **El primer mes acumula su flujo** | Quita la excepción del artefacto (`motor.py`: *"primer mes: caja fija"*). La caja de arranque pasa a ser un valor ANTERIOR al primer mes. | Candado | ⬜ Pendiente |
| **P4** | **Mes en curso = objetivo** | El mes en ejecución deja de anclar gasto real (muestra el presupuesto) y su rampa vuelve a ser la META, no el remanente. | Paso 1 | ⬜ Pendiente |
| **P5** | **Cronograma del mes completo** | La carga semanal conserva pagadas + parciales del mes en curso; corte de no-solape al cierre del mes anterior; cuota 0 nunca entra al recaudo. | Paso 1 | ⬜ Pendiente |
