...
| **2** | **Loop del agente + cifra→evidencia** | Loop con SDK Anthropic (temp 0.1, límites), verificador cifra→evidencia invocado antes de publicar, endpoint `/api/v1/cfo` bajo flag, salida tipada. Primeros eventos `cfo.*` (CR). | Kimi (crítico) + CR eventos | 🟢 **Construido + revisado** (SDD, `feat/fabs-inc2` `1955cb9`); pendiente gate Kimi + GO CEO para merge |
...
| — | 2→3 | **Pendiente de merge (gate de proceso):** paquete Kimi en `planning/phases/fabs-inc2/auditorias/PR1-I/` (merge crítico: toca catálogo de auditoría + lee plata) + **GO CEO** (gate-waiver como inc1, Kimi retroactivo) + update del tracker `.xlsx`. NO auto-merge. Al encender en inc3: `ANTHROPIC_API_KEY` en Render + cerrar la verificación concept-aware + respetar O-1 (no encender en prod sin auditoría retroactiva). | — |
