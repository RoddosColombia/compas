...
| **4** | **Visual · RV-V1 tokens + RV-V2 curva + composición** | RV-V1 `DESIGN.md §3` con paleta como tokens (`--color-chart-*`, contrato para RV-V2) · RV-V2 rehacer las 2 gráficas principales contra el mockup (rebanada 1: curva de caja, 7 de 10 AC · rebanada 2: composición separada AC #8 · rebanada 3: escenario superpuesto AC #5 + motos editable AC #7) | Kimi ausente ⇒ gate-waiver GO CEO. Gates finales: G-PIXEL (chrome-devtools-mcp + lost-pixel vs mockup) + G-AXE (accesibilidad) | 🟡 **RV-V1 ✅ mergeado. RV-V2 en curso (8 de 10 AC):** rebanada 1 ✅ mergeada (`031b1ad`); **rebanada 2 ✅ mergeada (`2e39c04`)** — composición del flujo en gráfica propia; rebanada 3 ⬜ pendiente (AC #7 depende de exponer el solver de unidades hoy en FABS). Ver §4 |
